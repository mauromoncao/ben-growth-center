// ============================================================
// BEN GROWTH CENTER — Webhook Meta Cloud API (WhatsApp Oficial)
// Rota: GET  /api/whatsapp-meta  → verificação do webhook
//       POST /api/whatsapp-meta  → mensagens recebidas
//
...
async function enviarMensagem(numero, texto) {
  if (!META_TOKEN || !META_PHONE_ID) {
    console.error('[Meta] TOKEN ou PHONE_ID não configurado')
    return
  }
  try {
    ...
    if (res.ok) {
      console.log('[Meta] Mensagem enviada para', numero, '— ID:', data?.messages?.[0]?.id)
    } else {
      console.error('[Meta] Erro ao enviar:', JSON.stringify(data).slice(0, 200))
    }
    return data
  } catch (e) {
...
async function crmCriarLead({ nome, telefone, numero, area, urgencia, resumo }) {
  const payload = { nome, telefone, numero, area, urgencia, resumo, canal: 'whatsapp-meta' }
  try {
...
// ── Sessões em memória ───────────────────────────────────────
if (!global.__drbenSessoesMeta)  global.__drbenSessoesMeta  = new Map()
if (!global.__drbenTriagemMeta)  global.__drbenTriagemMeta  = new Map()
...
async function maraAvisarDrMauro({ nome, telefone, numero, area, urgencia, resumo }) {
  ...
  await enviarMensagem(DR_MAURO_WHATSAPP.replace(/\D/g, ''), msg)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('[Meta Webhook] Payload:', JSON.stringify(body).slice(0, 300))

    // Extrair mensagem do payload Meta Cloud API
    ...
    const numero   = msg.from  // formato: 5511999998888
    const pushName = contact?.profile?.name ?? ''
    const texto    = msg?.text?.body ?? ''
    const tipo     = msg?.type ?? ''

    // Ignorar mensagens que não sejam texto
    if (tipo !== 'text' || !texto || !numero) {
      console.log('[Meta] Ignorando mensagem não-texto. Tipo:', tipo)
      return res.status(200).json({ ok: true })
    }
    ...
    if (ehDrMauro) {
      ...
      if (cmd === '/reset' || cmd === 'reset') {
        global.__drbenSessoesMeta.delete(numero)
        global.__drbenTriagemMeta.delete(numero)
        global.__drbenSessoesMeta.set(numero, [])
        global.__drbenTriagemMeta.set(numero, { nome: null, telefone: null, area: null, urgencia: null, notificado: false })
      ...
      if (cmd === '/status' || cmd === 'status') {
        const sessao = global.__drbenSessoesMeta.get(numero)
        const msg = [
          '📊 *Status Dr. Ben (Meta Cloud API)*',
          `• Sessões ativas: ${global.__drbenSessoesMeta.size}`,
      ...
      if (!global.__drbenSessoesMeta.has(numero)) {
    ...
    if (!global.__drbenTriagemMeta.has(numero)) {
      global.__drbenTriagemMeta.set(numero, {
        nome:      pushName || null,
        telefone:  null,
        area:      null,
        urgencia:  null,
    ...
    } else if (pushName && !global.__drbenTriagemMeta.get(numero).nome) {
      global.__drbenTriagemMeta.get(numero).nome = pushName
    }

    const history      = global.__drbenSessoesMeta.get(numero)
    const dadosTriagem = global.__drbenTriagemMeta.get(numero)
    ...
    // Limpar marcadores antes de enviar
    ...
    // MARA IA avisa Dr. Mauro UMA VEZ quando triagem completa
    ...
    return res.status(200).json({ ok: true }) // sempre 200 para Meta não retentar
  }
}
